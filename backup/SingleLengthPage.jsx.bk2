...
const CountByWidth = ({ title, length, selections, selectedWidth, handleSelection, handleIncrement, recordedData, disabled }) => {
    ...
    return (
        <div className={`p-6 bg-white rounded-xl shadow-lg ${disabled ? 'opacity-40 pointer-events-none' : ''}`}>
            <div className="flex justify-between items-center border-b-2 border-gray-200 pb-3 mb-4">
                <h2 className="text-2xl font-bold text-gray-700">{title}</h2>
                <div className="p-2 bg-gray-100 rounded-lg text-center text-base font-mono text-gray-800">
                    ...
                    <div key={rowIndex} className="grid grid-cols-7 gap-3">
                        {row.map(w => (
                            <button
                                ...
                                disabled={disabled}
                                className={`py-3 px-4 text-lg rounded-lg shadow-md font-bold transition-all transform hover:scale-105 ${selectedWidth === w && !disabled ? 'bg-indigo-600 text-white ring-4 ring-indigo-300' : 'bg-white hover:bg-gray-200'}`}
                            >
                                {w}
                            </button>
                        ))}
                ...
                <div className="grid grid-cols-10 gap-3">
                    {Array.from({ length: 10 }, (_, i) => i + 1).map(num => (
                        <button
                            ...
                            className="py-3 px-4 bg-blue-500 text-white font-bold rounded-lg shadow-lg hover:bg-blue-600 transition-all transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed"
                        >
                            x{num}
                        </button>
                    ))}
        ...
        <div className="bg-gray-100 text-gray-800 p-4 md:p-6 min-h-screen font-sans">
            <div className="max-w-screen-2xl mx-auto">
                <div className="relative flex justify-center items-center mb-6">
                    ...
                    <h1 className="text-4xl font-bold text-gray-800">Single Length Counter</h1>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                    ...
                    <div className="lg:col-span-3">
                        <div className={`p-6 bg-white rounded-xl shadow-lg transition-all ${locks.thickness && (locks.length1 || locks.length2) ? 'bg-green-50' : ''}`}>
                            <h2 className="text-2xl font-bold mb-4 text-center border-b-2 pb-3">Selections</h2>
                            <div className="space-y-4">
                                {/* Thickness */}
                                <div className="flex items-center space-x-2">
                                    <label className="font-semibold w-28 text-lg">Thickness:</label>
                                    <select
                                        value={selections.thickness}
                                        onChange={(e) => handleSelection('thickness', parseFloat(e.target.value))}
                                        disabled={locks.thickness}
                                        className="p-3 border rounded-lg w-full focus:ring-2 focus:ring-indigo-500 text-lg"
                                    >
                                        {thicknessData.map(t => <option key={t} value={t}>{t}</option>)}
                                    </select>
                                    <button onClick={() => handleLock('thickness')} className={`py-3 px-4 rounded-lg text-white flex items-center justify-center w-32 transition-colors text-base font-semibold ${locks.thickness ? 'bg-red-500 hover:bg-red-600' : 'bg-green-500 hover:bg-green-600'}`}>
                                ...
                                {/* Length 1 */}
                                <div className="flex items-center space-x-2">
                                    <label className="font-semibold w-28 text-lg">Length:</label>
                                    <select
                                        value={selections.length1Int}
                                        onChange={(e) => handleSelection('length1Int', parseInt(e.target.value))}
                                        disabled={locks.length1}
                                        className="p-3 border rounded-lg w-full focus:ring-2 focus:ring-indigo-500 disabled:opacity-50 text-lg"
                                    >
                                    ...
                                    <button onClick={() => handleLock('length1')} className={`py-3 px-4 rounded-lg text-white flex items-center justify-center w-32 transition-colors text-base font-semibold ${locks.length1 ? 'bg-red-500 hover:bg-red-600' : 'bg-green-500 hover:bg-green-600'}`}>
                                ...
                                <div className="flex items-center space-x-2">
                                    ...
                                    <button onClick={() => handleLock('length2')} className={`py-3 px-4 rounded-lg text-white flex items-center justify-center w-32 transition-colors text-base font-semibold ${locks.length2 ? 'bg-red-500 hover:bg-red-600' : 'bg-green-500 hover:bg-green-600'}`}>
                ...
                <div className="mt-8 flex justify-center">
                    <button 
                        ...
                        className="py-4 px-8 bg-green-600 text-white text-xl rounded-lg hover:bg-green-700 font-semibold shadow-lg transition-all transform hover:scale-105"
                    >
                        Finish & Save Report
                    </button>
                </div>
